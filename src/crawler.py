    ...
    def process_submissions(self, submissions):
        ...
        for submission in submissions:
            ...
            try:
                ...
                if fail_count == 2:
                    logger.warning("Code continually getting None. It may caused by service banning, wait minutes to continue.")
                    break
                fail_count += 1
    ...
    def process_submission(self, submission):
        ...
        if submission_token not in self.visited:
            ...
            if not self.OVERWRITE and os.path.exists(full_path):
                return
            self.save_code(submission_details["code"], problem_frontendId, problem_title, submission_lang, full_path)
    ...
    def save_code(self, code, problem_frontendId, problem_title, submission_lang, full_path):
        with open(full_path, "w") as f:  # 开始写到本地
            ...
            if self.is_temporary_problem(problem_frontendId):
                self.problems_to_be_reprocessed.append((full_path, problem_title, submission_lang))
            ...
            with open(TEMP_FILE_PATH, "r") as f:
                for line in f.readlines():
                    try:
                        ...
                        logger.warning("Your " + TEMP_FILE_PATH + " is in old format, delete all temp code.")
        ...
        if self.problems_to_be_reprocessed:
            with open(TEMP_FILE_PATH, "w") as f:
                for full_path, problem_title, submission_lang in self.problems_to_be_reprocessed:
                    f.write(full_path + " " + problem_title + " " + submission_lang + "\n")
